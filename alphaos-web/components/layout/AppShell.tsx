...
function Brand() {
  return <Link href="/" className="flex items-center gap-2.5"><div className="flex h-9 w-9 items-center justify-center rounded-xl border border-emerald-400/20 bg-emerald-400/10 text-xs font-bold text-emerald-300">A</div><div><p className="text-sm font-semibold tracking-tight text-white">AlphaOS <span className="text-emerald-300">AI</span></p><p className="text-[8px] uppercase tracking-[0.14em] text-zinc-600">Crypto Intelligence</p></div></Link>;
}
...
export default function AppShell({ children }: AppShellProps) {
  ...
  return <div className="min-h-screen bg-[#050609] text-white">
    ...
    <header className="sticky top-0 z-30 flex h-[60px] items-center justify-between border-b border-white/[0.07] bg-[#050609]/90 px-4 backdrop-blur-xl lg:hidden"><Brand /><SystemStatus /></header>
    <div className="lg:pl-[250px]">{children}</div>
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/[0.08] bg-[#07090c]/95 px-2 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl lg:hidden"><div className="mx-auto grid max-w-md grid-cols-5 gap-1">{navigationItems.map((item) => { const active = isActiveRoute(pathname, item.href); return <Link key={item.href} href={item.href} className={`flex min-h-12 flex-col items-center justify-center rounded-xl text-[9px] font-medium transition ${active ? "bg-emerald-400/[0.08] text-emerald-300" : "text-zinc-600"}`}><span className="mb-0.5 text-base leading-none">{item.icon}</span>{item.label}</Link>; })}</div></nav>
  </div>;
}
