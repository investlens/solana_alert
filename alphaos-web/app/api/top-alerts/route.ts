...
export async function GET(){
 try{
  ...
  const [solAll,rhAll,solRecent,rhRecent]=await Promise.all([
   ...
   supabaseAdmin.from("alerts").select(solSelect).order("alerted_at",{ascending:false}).limit(20).abortSignal(AbortSignal.timeout(3500)),
   supabaseAdmin.from("robinhood_observations").select(rhSelect).not("alerted_at","is",null).order("alerted_at",{ascending:false}).limit(20).abortSignal(AbortSignal.timeout(3500))
  ]);
  ...
  const recent=[...(solRecent.data??[]).map(mapSolana),...(rhRecent.data??[]).map(mapRobinhood)].sort((a,b)=>new Date(b.alertedAt??0).getTime()-new Date(a.alertedAt??0).getTime()).slice(0,10);
