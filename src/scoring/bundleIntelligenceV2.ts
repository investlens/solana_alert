...
type CacheEntry = { expiresAt: number; value: BundleIntelligenceV2Result };

const connection = new Connection(config.solanaRpcUrl, 'confirmed');
const CACHE_TTL_MS = 60_000;
const cache = new Map<string, CacheEntry>();
const inflight = new Map<string, Promise<BundleIntelligenceV2Result>>();
...
async function resolveLargestHolderWallets(mintAddress: string): Promise<HolderWallet[]> {
  ...
  // Resolve token-account owners sequentially instead of bursting 12 RPC calls at once.
  for (let index = 0; index < tokenAccounts.length; index += 1) {
    const tokenAccount = tokenAccounts[index];
    try {
      const account = await connection.getParsedAccountInfo(tokenAccount.address, 'confirmed');
      ...
      if (!owner) continue;
      const amount = Number(tokenAccount.uiAmountString ?? tokenAccount.uiAmount ?? 0);
      if (!Number.isFinite(amount) || amount <= 0) continue;
      const sharePct = (amount / totalSupply) * 100;
      walletShares.set(owner, (walletShares.get(owner) ?? 0) + sharePct);
    } finally {
      if (index < tokenAccounts.length - 1) await delay(75);
    }
  }

  return [...walletShares.entries()]
    .map(([wallet, sharePct]) => ({ wallet, sharePct }))
...
async function inspectWallet(holder: HolderWallet): Promise<WalletFundingEvidence> {
  ...
  if (fresh && signatures.length) {
    ...
    for (let index = 0; index < likelyFunding.length; index += 1) {
      const sig = likelyFunding[index];
      try {
        ...
        if (index < likelyFunding.length - 1) await delay(60);
      }
    }
  }
...
async function calculateBundleIntelligenceV2(mintAddress: string): Promise<BundleIntelligenceV2Result> {
  try {
    ...
    if (clusterCandidates.length < 3) {
      return { ...emptyResult('Not enough distributed holder wallets for bundle analysis'), evidenceAvailable: true, metrics: { ...emptyMetrics(), holdersResolved: resolved.length } };
    }

    const inspected: WalletFundingEvidence[] = [];
    // Wallet-history inspection is intentionally sequential. This path only runs for
    // safety analysis and should favor reliable evidence over RPC burst throughput.
    for (let index = 0; index < clusterCandidates.length; index += 1) {
      inspected.push(await inspectWallet(clusterCandidates[index]));
      if (index < clusterCandidates.length - 1) await delay(100);
    }
    ...
    for (const wallet of inspected) {
      for (const funder of wallet.funders) {
    ...
    const result: BundleIntelligenceV2Result = {
      ...
      reasons,
      connectedWallets: strongestConnectedWallets,
      metrics: {
        holdersResolved: resolved.length,
        freshWallets: freshWallets.length,
        freshWalletCombinedSharePct: round2(freshWalletCombinedSharePct),
        largestCommonFunderWalletCount,
        largestCommonFunderSharePct: round2(largestCommonFunderSharePct),
        inspectedCombinedSharePct: round2(inspectedCombinedSharePct),
      },
    };
...
export async function getBundleIntelligenceV2(mintAddress: string): Promise<BundleIntelligenceV2Result> {
  ...
  if (cached && cached.expiresAt > now) {
    console.log('[BundleIntelligenceV2] cache hit', { mintAddress, evidenceAvailable: cached.value.evidenceAvailable, score: cached.value.score });
    return cached.value;
  }

  const existing = inflight.get(mintAddress);
  if (existing) {
    console.log('[BundleIntelligenceV2] inflight dedup', { mintAddress });
    return existing;
  }

  const task = calculateBundleIntelligenceV2(mintAddress)
    .then((value) => {
      // Cache successful evidence for 60s. Cache unavailable results only briefly so
      // transient RPC throttling gets a quick retry without creating a retry storm.
      const ttl = value.evidenceAvailable ? CACHE_TTL_MS : 10_000;
      cache.set(mintAddress, { expiresAt: Date.now() + ttl, value });
      return value;
    })
    .finally(() => {
      ...
      if (cache.size > 500) {
        ...
        for (const [key, entry] of cache.entries()) if (entry.expiresAt <= cutoff) cache.delete(key);
      }
    });

  inflight.set(mintAddress, task);
  return task;
}
