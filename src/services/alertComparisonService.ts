...
const POSITIVE_TYPES = new Set(['OPPORTUNITY', 'ENTRY', 'CHECK_ENTRY', 'VOLUME_SURGE', 'DEX_PAID', 'BOOST', 'MAJOR_BOOST', 'DEV_BURN']);
...
export function buildAlertComparison(previous: ComparableAlertEvent | null, current: ComparableAlertEvent): AlertComparison {
  ...
  if (!POSITIVE_TYPES.has(currentType)) return { hasPriorAlert: false, currentState: current.intelligence_state ?? undefined, ...identity };
...
export async function loadPriorDeliveredAlertComparison(args: { currentEventId: number; assetId: string; chain: string }): Promise<AlertComparison> {
  ...
  const fields = 'id,alerted_at,asset_id,symbol,token_name,semantic_event_type,alert_type,intelligence_state,price,price_provenance,market_cap,liquidity,volume_5m';
  ...
  const positiveCandidates = (candidates ?? []).filter(row => POSITIVE_TYPES.has(String(row.semantic_event_type ?? row.alert_type ?? '').toUpperCase()));
