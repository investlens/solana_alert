...
async function scanToken(entry: ExistingTokenUniverseEntry) {
  ...
  const market = await getRobinhoodMarketSnapshot(entry.token, { priority: 'BACKGROUND', caller: 'existing_token_scanner',
    queueWaitTimeoutMs: EXISTING_TOKEN_SCANNER_QUEUE_WAIT_MS });
  if (!market) { recordDexScreenerCallerOutcome('existing_token_scanner', 'BACKGROUND', 'NO_USABLE_PAIR');
    throw new ExistingTokenNoUsablePairError(); }
...
export async function refreshExistingTokenOpportunityScanner() {
  ...
  const metrics = { health: 'HEALTHY', universe: 0, due: 0, selected: 0, scanned_success: 0, no_market: 0,
    failed: 0, queue_deferred: 0, cycle_deferred: 0, provider_backoff_deferred: 0, remaining_due: 0,
    candidates: 0, qualified: 0, actionable_emitted: 0, failure_reasons: {} as Record<string, number>,
    provider_backoff: false, duration_ms: 0 };
  try {
    ...
    metrics.due = due.dueCount; metrics.selected = due.selected.length;
    metrics.queue_deferred = Math.max(0, due.dueCount - due.selected.length);
    ...
    if (getDexScreenerBackoffState().active) {
      metrics.health = 'DEGRADED'; metrics.failed = 1; metrics.provider_backoff = true;
      metrics.provider_backoff_deferred = due.selected.length; metrics.failure_reasons.RATE_LIMITED = 1;
      metrics.remaining_due = due.dueCount;
      return metrics;
    }
    ...
    for (const { result } of batch.completed) { metrics.scanned_success++;
    ...
    for (const { entry, error } of batch.failed) { const reason = error instanceof Error ? error.message : String(error);
      const category = error instanceof DexScreenerQueueCapacityError ? 'DEFERRED_QUEUE_CAPACITY'
        : error instanceof ExistingTokenNoUsablePairError ? 'NO_USABLE_PAIR'
        : error instanceof DexScreenerHttpTimeoutError ? 'HTTP_TIMEOUT'
        : error instanceof DexScreenerMalformedResponseError ? 'MALFORMED_RESPONSE'
        : error instanceof DexScreenerProviderHttpError ? 'PROVIDER_HTTP_ERROR'
        : isDexScreenerProviderBackoffError(error) ? 'RATE_LIMITED'
        : reason.startsWith('scanner persistence failed') ? 'PERSISTENCE_FAILED' : 'TOKEN_SCAN_FAILED';
      if (category === 'DEFERRED_QUEUE_CAPACITY') metrics.queue_deferred++;
      else if (category === 'NO_USABLE_PAIR') metrics.no_market++;
      else metrics.failed++;
    ...
    if (batch.providerBackoff) { metrics.provider_backoff = true; metrics.failed += 1;
      metrics.failure_reasons.RATE_LIMITED = 1; metrics.provider_backoff_deferred = batch.skipped; }
    else if (batch.skipped) { metrics.cycle_deferred += batch.skipped;
      metrics.failure_reasons.CYCLE_BUDGET_DEFERRED = batch.skipped; }
    metrics.remaining_due = Math.max(0, due.dueCount - metrics.scanned_success);
    metrics.health = metrics.failed ? 'DEGRADED'
      : metrics.queue_deferred || metrics.cycle_deferred ? 'CAPACITY_LIMITED' : 'HEALTHY';
    return metrics;
  } catch (error) { metrics.failed++; metrics.health = 'DEGRADED'; console.error('[ExistingTokenScanner] cycle failed', { reason: error instanceof Error ? error.message : String(error) }); return metrics; }
