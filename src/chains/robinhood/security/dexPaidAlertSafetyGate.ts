...
function push(checks: GateCheck[], reasons: string[], key: string, passed: boolean, detail: string) {
  ...
  if (!passed) reasons.push(`${key}: ${detail}`);
}
...
export async function evaluateDexPaidAlertSafety(tokenAddress: string): Promise<DexPaidAlertSafetyResult> {
  ...
  try {
    const [market, dexPaid, contractGate, adminRisk, launch] = await Promise.all([
      ...
      scanRobinhoodDexPaid(tokenAddress),
    ...
    const pairAgeMinutes = pairCreatedAt != null
      ? Math.max(0, (Date.now() - pairCreatedAt) / 60_000)
      : null;
    ...
    const dangerousAdminSignals = adminRisk.signals.filter(signal =>
    ...
    if (!market || !market.pairAddress) {
      push(checks, reasons, 'POOL', false, 'verified pair unavailable');
    } else {
    ...
    if (isVerifiedPonsLaunch) {
      const sellability = await scanPonsSellability(tokenAddress);
      sellImpact = finite(sellability.estimatedImpactPercent);
      const sellPass = sellability.sellable === true && sellability.status === 'SELLABLE' &&
        (sellImpact == null || sellImpact <= MAX_SELL_IMPACT_PERCENT);
      push(checks, reasons, 'SELLABILITY', sellPass,
        `status=${sellability.status}${sellImpact == null ? '' : `, impact=${sellImpact.toFixed(1)}%`}`);
    } else {
      push(checks, reasons, 'SELLABILITY', false, 'sell route cannot be verified as PONS');
    }

    const holder = await scanRobinhoodHolderRisk(tokenAddress, { poolAddress: market?.pairAddress ?? null, timeoutMs: 2_750 });
    const holderPass = holder.concentrationRisk !== 'HIGH' &&
      (holder.top1Pct == null || holder.top1Pct <= MAX_TOP1_PERCENT);
    ...
    if (isVerifiedPonsLaunch) {
      const [devHolding, devMovement] = await Promise.all([
        scanRobinhoodDevHolding(tokenAddress),
        scanRobinhoodDevMovement(tokenAddress),
      ]);
      ...
      push(checks, reasons, 'DEV_MOVEMENT', movementPass,
        `status=${devMovement.status}${devMovement.burned ? ', burn-only' : ''}`);
    }
    ...
    return {
      ...
      pairAgeMinutes,
      sellImpactPercent: sellImpact,
      ponsDeployer: launch?.exists ? String(launch.deployer) : null,
    };
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error);
    return {
      allowed: false,
      reasons: [`SAFETY_GATE_ERROR: ${reason}`],
      checks: [{ key: 'SAFETY_GATE_ERROR', passed: false, detail: reason }],
      marketCapUsd: null,
      liquidityUsd: null,
      pairAgeMinutes: null,
      sellImpactPercent: null,
      ponsDeployer: null,
    };
  }
}
